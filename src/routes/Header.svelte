...
<div class="z-20 absolute top-3 right-3">
  <button
    ...
    class="w-10 h-10 rounded-full text-f4 overflow-hidden flex items-center justify-center bg-black/10 hover:bg-black/20 transition-colors dark:bg-white/10 dark:hover:bg-white/20"
  >
    {#if !darkMode}
      <svg 
  ...
  {#if $open}
    <div
      ...
      class="z-10 rounded-lg bg-white shadow dark:bg-dark-lighten1 dark:text-white"
    >
      <div use:melt={$arrow} />
      <p class="px-4 py-1 text-magnum-700 dark:text-white">
